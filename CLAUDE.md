...
**Game concept:** Players join an arena → countdown starts when minimum players reached → each player receives a theme (topic) → player builds in their plot zone → every 5 seconds the system renders the dirty plot mirror and runs ML classification → if the theme is in the top-K predictions the player scores a point and the zone is cleared, a new theme is assigned → when the game timer expires, the player with most points wins. Players must build *recognizably for a visual classifier*, not just aesthetically.
...
- `SceneData` — thread-safe interface for block access (implementations: `ChunkScene` for live world snapshots, `FlatScene` for flat-array scenes from schematics or snapshots, `MutablePlotScene` for per-plot event-driven mirrors used by the game render tick).
- `MutablePlotScene` (in `render/data/`) — per-plot scene mirror that replaces main-thread `RenderService.capture(region)` in the game render tick. Backed by a flat `short[]` of `XMaterial` ordinals + parallel `byte[] legacyBlockData` (1.8–1.12) **or** `String[] blockStates` (1.13+), selected via the `legacy` constructor flag. **Threading model:** single writer (the main Bukkit thread, via `GameListener` → `GameManager.applyMirrorPlace`/`applyMirrorBreak`) + multiple readers (the async render task). Cell writes (`setBlock`, `clearBlock`) are lock-free — they rely on JLS §17.7 atomic single-store for `short`/`byte`/reference; render reads take a `readLock()` around the entire frame. Only `clearAll()` (called from `handleScore` and on theme change) takes the write-lock. State-string allocations are gated on `BlockPalette.needsBlockState(material)` — stateless blocks (stone, wool, glass …) never allocate a string. Out-of-bounds writes silently no-op; size validated against `MAX_HEIGHTMAP_AREA`-style caps in the constructor. Factory `forPlot(Arena.PlotData, boolean legacy)` normalises corners via `Math.min`/`Math.max`.
...
- **`GameSession`** (package-private) — tracks one arena's active session: state, players (`Map<UUID, GamePlayer>`), used plot indices, shuffled theme list, rotating camera index (cycles 0→1→2→0…), remaining game time, the four scheduler task IDs (countdown / game tick / render / ending) so `cancelAllTasks()` cleans up reliably on shutdown, and a `Map<Integer, MutablePlotScene> mirrors` keyed by plot index. Exposes package-visible `mirror(plotIndex)`, `installMirror(plotIndex, scene)`, and `clearMirrors()` for the `GameManager` lifecycle hooks.
...
- **Timers** (Bukkit scheduler):
  ...
  - Render/ML: every `RENDER_INTERVAL_TICKS = 100` (5 s). For each player whose zone is dirty: the main thread grabs the player's `MutablePlotScene` mirror, clears the dirty flag eagerly, and dispatches `runTaskAsynchronously`. The async task takes `mirror.readLock()` around `RenderService.render(mirror, camX, camY, camZ, yaw, pitch)`, releases the lock, then calls `MLService.predictRgb(rgb, 224, 224, 2)` outside the lock. **The main thread never calls `RenderService.capture(region)` from the render tick** — the mirror has already accumulated event-driven place/break updates with zero per-tick Bukkit-API cost. If the player's theme is in the top-2, `handleScore` is scheduled back on the main thread to bump score, clear the zone (and call `mirror.clearAll()` under its write-lock), advance the theme, and re-arm the build timer. Camera rotates between the plot's 3 cameras per render tick (session-wide, not per-player).
...
- **GameManager exposes package-visible helpers used by `GameListener`** that are not part of `BBAIGameManager`: `getPlayerPlotIndex(uuid, arenaName)`, `markPlayerZoneDirty(uuid, arenaName)`, `applyMirrorPlace(uuid, arenaName, placedBlock)`, and `applyMirrorBreak(uuid, arenaName, brokenBlock)`. The mirror helpers branch on the cached `legacy` flag — calling either `mirror.setBlock(..., placedBlock.getData())` on 1.8–1.12 or `mirror.setBlock(..., placedBlock.getBlockData().getAsString())` on 1.13+. There's also a static `isInZone(x, y, z, plot)` AABB check shared between the listener and the manager itself.
- **Mirror lifecycle:** mirrors are installed in `startGame` (one per player plot via `MutablePlotScene.forPlot(plot, legacy)`), `clearAll()`-ed in `handleScore`, and bulk-disposed via `session.clearMirrors()` in `endGame` and `forceEndSession`.
- **Forced-shutdown semantics:** `GameManager.shutdown()` walks every active session, cancels all timers, clears every plot zone, restores every player's snapshot, and clears the maps (including mirrors). Invariant: after a plugin reload the world is exactly as if no game was ever in progress.
...
- `render/` — `RenderService`, `CpuRenderer`, `BlockPalette`, `BlockShape`, `BlockRenderState`; `render/data/` has `ChunkScene`, `FlatScene`, `MutablePlotScene`, `SceneData`, `LegacyBlockStates`
...
- **Plot mirror writes** (`MutablePlotScene.setBlock`/`clearBlock` via `GameManager.applyMirrorPlace`/`Break`) **must run on the main thread.** Single-writer/multi-reader is the entire safety contract — never write to a mirror from a render thread or async task. Reads from any thread must take `mirror.readLock()` for the duration of the read (the renderer holds it for the whole frame). `clearAll()` already takes the write-lock internally.
...
- `GameManagerTest` / `GameSessionTest` / `GamePlayerTest` / `PlayerSnapshotTest` cover the session lifecycle in isolation: state transitions, plot assignment, scoring, mirror install/clear lifecycle, snapshot deep-cloning, off-hand gating on 1.9+ vs 1.8. Mock the ML and render services to keep them deterministic.
- `MutablePlotSceneTest` covers the synchronous write API (AIR init, set/clear transitions, out-of-bounds no-op, legacy byte mode, `forPlot` corner normalisation, `clearAll` resets all cells, `readLock()` exposure). `MutablePlotSceneConcurrencyTest` covers the concurrency contract: a write attempt on `clearAll` blocks while a reader holds the read-lock, and a 1-writer × 4-readers stress run (5000 set/clear ops × 50 full scans) observes only AIR or STONE — never torn intermediate values. Add new mirror tests in this pattern, not via reflection.
- `GameListenerTest` mocks `BBAIGameManager` via the **concrete `GameManager` class** (not the interface) because the listener casts to call package-visible helpers (`getPlayerPlotIndex`, `markPlayerZoneDirty`, `applyMirrorPlace`, `applyMirrorBreak`). New listener tests that exercise mirror code must follow this pattern.
