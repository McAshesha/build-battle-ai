...
public interface BBAINPCService {
     ...
     */
    @NonNull
    NPCService.NPC createNPC(@NonNull String name, @NonNull Location location,
     ...
     */
    @NonNull
    NPCService.NPC createNPC(@NonNull Player skinSource, @NonNull String name, @NonNull Location location);
     ...
     */
    @NonNull
    NPCService.NPC createNPC(@NonNull String skinName, @NonNull String name, @NonNull Location location);

    /**
     * Shows the NPC to the specified viewer.
     * <p>
     * Sends player-info, spawn, skin-layers metadata, head-rotation, and any
     * stored equipment packets. The tab list entry is automatically removed
     * after a short delay so the client has time to download the skin texture.
     *
     * @param viewer the player who will see the NPC
     * @param npc    the NPC to display
     */
    void spawn(@NonNull Player viewer, @NonNull NPCService.NPC npc);

    /**
     * Shows the NPC to the specified viewers.
     *
     * @param viewers the players who will see the NPC
     * @param npc     the NPC to display
     * @see #spawn(Player, NPCService.NPC)
     */
    void spawn(@NonNull Collection<Player> viewers, @NonNull NPCService.NPC npc);
     ...
     * The equipment is also stored on the NPC so that future {@link #spawn}
