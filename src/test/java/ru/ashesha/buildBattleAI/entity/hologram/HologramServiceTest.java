...
class HologramServiceTest {
    ...
    void colorizeTranslatesAmpersandCodes() {
        assertEquals("\u00a7aGreen \u00a7bAqua", HologramService.colorize("&aGreen &bAqua"));
    }

    @Test
    void colorizeHandlesFormatCodes() {
        assertEquals("\u00a7lBold \u00a7oItalic", HologramService.colorize("&lBold &oItalic"));
    }

    @Test
    void colorizeIgnoresInvalidCodes() {
        // 'x' and 'z' are not valid color/format codes
        assertEquals("&xInvalid &zTest", HologramService.colorize("&xInvalid &zTest"));
    }

    @Test
    void colorizeHandlesUpperCaseCodes() {
        assertEquals("\u00a7AGreen \u00a7BBold", HologramService.colorize("&AGreen &BBold"));
    }

    @Test
    void colorizeEmptyString() {
        assertEquals("", HologramService.colorize(""));
    }

    @Test
    void colorizeNull() {
        assertEquals("", HologramService.colorize(null));
    }

    @Test
    void colorizeNoAmpersand() {
        assertEquals("Plain text", HologramService.colorize("Plain text"));
    }

    @Test
    void colorizeTrailingAmpersand() {
        // Lone ampersand at the end should not cause issues
        assertEquals("Text&", HologramService.colorize("Text&"));
    }

    @Test
    void colorizeResetCode() {
        assertEquals("\u00a7rReset", HologramService.colorize("&rReset"));
    }

    @Test
    void colorizeFastPathReturnsOriginalInstance() {
        // When there are no '&' characters, the same string instance should be returned
        String input = "No ampersands here";
        assertSame(input, HologramService.colorize(input));
    }
}
